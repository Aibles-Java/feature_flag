...
/**
 * Unit tests for {@link FeatureFlagMetrics}, asserting each business meter is registered under
 * the expected Micrometer name with bounded tags, and that the evaluation wrapper both runs the
...
class FeatureFlagMetricsTest {
    ...
    void recordEvaluation_runsWorkAndRecordsCountAndTimer() {
        ...
        assertThat(registry.timer("ff.evaluation.duration", "environment", "env-1").count()).isEqualTo(1L);
    }
    ...
    void recordFlagChange_incrementsPerBoundedChangeType() {
        ...
        assertThat(registry.counter("ff.flag.changes", "change", "state_updated").count()).isEqualTo(2.0);
    }
    ...
    void recordAuthFailure_tagsChainAndReason() {
        ...
        assertThat(registry.counter("ff.auth.failures", "chain", "sdk", "reason", "invalid_key").count())
                .isEqualTo(1.0);
        assertThat(registry.counter("ff.auth.failures", "chain", "admin", "reason", "unknown_subject").count())
                .isEqualTo(1.0);
    }
}
