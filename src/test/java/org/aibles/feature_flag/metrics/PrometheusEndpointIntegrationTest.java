...
/**
 * Verifies the acceptance criteria of issue #29 end-to-end through the real management {@code
 * SecurityFilterChain}: {@code /actuator/prometheus} is not reachable without the scraper
 * credential, is scrapable with it, and includes the custom {@code ff_*} meters. Health stays
 * public. Uses the shared {@code test} context (credential set in {@code
 * application-test.properties}) so no second context is forked.
 */
@SpringBootTest
@ActiveProfiles("test")
class PrometheusEndpointIntegrationTest {
  ...
  void prometheusRejectsWrongCredentials() throws Exception {
    mockMvc
        .perform(get("/actuator/prometheus").with(httpBasic(SCRAPER_USER, "wrong-password")))
  ...
  void prometheusScrapesWithScraperCredential() throws Exception {
    mockMvc
        .perform(get("/actuator/prometheus").with(httpBasic(SCRAPER_USER, SCRAPER_PASS)))
  ...
  /**
   * Minimal HTTP Basic post-processor. Kept local to avoid importing {@code
   * SecurityMockMvcRequestPostProcessors.httpBasic}, which resolves credentials against the app's
   * user store rather than sending a raw Authorization header.
   */
