...
/**
 * Startup validation for {@link JwtProperties}. Uses ApplicationContextRunner, which
 * exercises the same ConfigurationProperties binding + Bean Validation path as real
 * application startup: an invalid secret must abort context creation (fail fast).
 */
class JwtPropertiesValidationTest {
    ...
    void missingSecretFailsStartup() {
        runner.withPropertyValues("app.jwt.expiration-ms=86400000")
                .run(context -> {
