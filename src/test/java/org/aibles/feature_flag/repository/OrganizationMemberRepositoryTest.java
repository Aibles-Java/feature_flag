...
class OrganizationMemberRepositoryTest {
  ...
  void setUp() {
    userA =
        persist(
            User.builder()
                .email("a-" + System.nanoTime() + "@example.com")
                .passwordHash("h")
                .build());
    userB =
        persist(
            User.builder()
                .email("b-" + System.nanoTime() + "@example.com")
                .passwordHash("h")
                .build());
    orgX = persist(Organization.builder().name("OrgX").slug("orgx-" + System.nanoTime()).build());
    orgY = persist(Organization.builder().name("OrgY").slug("orgy-" + System.nanoTime()).build());
  }

  private OrganizationMember member(User user, Organization org, MemberRole role) {
    return persist(OrganizationMember.builder().user(user).organization(org).role(role).build());
  }
