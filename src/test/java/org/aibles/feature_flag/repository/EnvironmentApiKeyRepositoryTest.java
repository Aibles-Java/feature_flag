...
/**
 * Deliberately NOT {@code @Transactional}. {@code touchLastUsedAt} is {@code REQUIRES_NEW} (see its
 * Javadoc on {@link EnvironmentApiKeyRepository}), which suspends whatever transaction is already
 * open and commits independently of it. Wrapping this test class in one rollback-only transaction
 * would suspend *that* transaction for every {@code touchLastUsedAt} call — and since it is never
 * committed, the fixture rows created in {@code setUp()} are invisible to the new transaction the
 * update runs in (H2, like PostgreSQL, does not let one transaction see another's uncommitted
 * writes). Every repository call here runs in, and commits, its own transaction instead, matching
 * {@code FlagHygieneIntegrationTest}, the other place in this codebase that exercises a {@code
 * REQUIRES_NEW} write end to end.
 */
@SpringBootTest(webEnvironment = SpringBootTest.WebEnvironment.NONE)
@ActiveProfiles("test")
@TestPropertySource(
    properties = {
      "spring.datasource.url=jdbc:h2:mem:testdb_envkey;DB_CLOSE_DELAY=-1;DB_CLOSE_ON_EXIT=FALSE;MODE=PostgreSQL;DATABASE_TO_LOWER=TRUE;NON_KEYWORDS=KEY,VALUE;CASE_INSENSITIVE_IDENTIFIERS=TRUE"
    })
class EnvironmentApiKeyRepositoryTest {
  ...
  @Test
  void findActiveByKeyHashSkipsAKeyExpiringAtExactlyNow() {
    // Mirrors EnvironmentApiKeyTest#keyIsExpiredAtTheExactExpiryInstant: the JPQL predicate
    // (expiresAt > :now) must draw the same closed boundary as the Java isExpired(Clock) check,
    // or the two could silently drift apart.
    save("ios", "eeee5555", now, null);

    assertThat(repository.findActiveByKeyHash("eeee5555", now)).isEmpty();
  }
  ...
  @Test
  void findAllByEnvironmentIdReturnsAPageOfThatEnvironmentsKeys() {
    // findAllByEnvironmentId carries no ORDER BY; the production caller supplies its own
    // (createdAt ascending), so this only asserts pagination scopes correctly to the
    // environment — it makes no ordering claim.
