...
class OrganizationControllerTest {
    ...
    void create_returns201_withOrgResponse() throws Exception {
        OrganizationResponse response = OrganizationResponse.builder()
                .id(UUID.randomUUID()).name("Acme").slug("acme").build();
    ...
    void get_returns200_withOrg() throws Exception {
        ...
        OrganizationResponse response = OrganizationResponse.builder()
                .id(orgId).name("Acme").slug("acme").build();
    ...
    void listMembers_returns200_withMemberList() throws Exception {
        ...
        MemberResponse member = MemberResponse.builder()
                .userId(UUID.randomUUID()).email("alice@example.com").role(MemberRole.OWNER).build();
        when(organizationService.listMembers(orgId)).thenReturn(List.of(member));

        mockMvc.perform(get("/api/v1/organisations/{orgId}/members", orgId))
                .andExpect(status().isOk())
    ...
    void inviteMember_returns201_withMemberResponse() throws Exception {
        ...
        MemberResponse member = MemberResponse.builder()
                .userId(newUserId).email("bob@example.com").role(MemberRole.ADMIN).build();
    ...
    void update_returns200_withUpdatedOrganisation() throws Exception {
        UUID orgId = UUID.randomUUID();
        OrganizationResponse response = OrganizationResponse.builder()
                .id(orgId).name("Renamed Corp").slug("acme").build();
        ...
        mockMvc.perform(put("/api/v1/organisations/{orgId}", orgId)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(req)))
                .andExpect(status().isOk())
