...
class EvaluationControllerTest {
  ...
  void setUp() {
    mockMvc =
        MockMvcBuilders.standaloneSetup(new EvaluationController(evaluationService))
  ...
  void getAllFlags_returns200_withFlagList() throws Exception {
    ...
    FlagEvaluationResponse flag =
        FlagEvaluationResponse.builder()
            .flagKey("dark-mode")
            .enabled(true)
    ...
    mockMvc
        .perform(withAuth(get("/api/v1/sdk/flags"), env))
  ...
  void getFlag_returns200_withFlagEvaluation() throws Exception {
    ...
    mockMvc
        .perform(withAuth(get("/api/v1/sdk/flags/{flagKey}", "beta-feature"), env))
        .andExpect(status().isOk())
  ...
  void getAllFlags_passesIdentifierToService() throws Exception {
    ...
    mockMvc
        .perform(withAuth(get("/api/v1/sdk/flags").param("identifier", "user-42"), env))
        .andExpect(status().isOk())
