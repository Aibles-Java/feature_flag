...
/**
 * Integration tests for the rate limiter (issue #26), exercising both security chains through
 * the real filter stack. Rate limiting is enabled here with a low capacity (2) via test
 * properties, overriding the {@code app.rate-limit.enabled=false} default of the test profile.
 *
...
@SpringBootTest(properties = {
        ...
        // other integration tests ("DATABASECHANGELOG already exists"). See rate-limit test notes (#26).
...
class RateLimitIntegrationTest {
    ...
    void authLoginIsRateLimitedPerIpWith429AndRetryAfter() throws Exception {
        ...
        for (int i = 0; i < CAPACITY; i++) {
            int status = mockMvc.perform(post(AUTH_LOGIN)
    ...
    void sdkEvaluationIsRateLimitedPerKeyWith429AndRetryAfter() throws Exception {
        ...
        for (int i = 0; i < CAPACITY; i++) {
            int status = mockMvc.perform(get(SDK_ENDPOINT).header("X-Environment-Key", apiKey))
                    .andReturn().getResponse().getStatus();
            assertThat(status).isNotEqualTo(HttpStatus.TOO_MANY_REQUESTS.value());
        }

        // The next request with the same key is throttled.
        mockMvc.perform(get(SDK_ENDPOINT).header("X-Environment-Key", apiKey))
                .andExpect(status().isTooManyRequests())
                .andExpect(header().exists("Retry-After"));
    }
