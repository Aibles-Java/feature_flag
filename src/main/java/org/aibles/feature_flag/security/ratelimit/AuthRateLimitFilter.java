...
/**
 * Per-IP rate limit on {@code /api/v1/auth/**} (brute-force protection, issue #26). These endpoints
 * are unauthenticated ({@code permitAll}), so the only stable client identifier is the source IP.
 *
 * <p>The key is {@link HttpServletRequest#getRemoteAddr()} — the direct TCP peer, which a client
 * cannot spoof. If the app is deployed behind a reverse proxy/load balancer, set {@code
 * server.forward-headers-strategy=framework} so {@code getRemoteAddr()} reflects the real client
 * instead of the proxy (we deliberately do NOT trust a raw {@code X-Forwarded-For} header, which an
 * attacker could forge to rotate the key and bypass the limit).
 */
