...
/**
 * In-memory token-bucket rate limiter (issue #26, v1). Holds one {@link Bucket} per client key
 * per scope, created lazily from {@link RateLimitProperties}.
 *
 * <p>Buckets are stored in a {@link Caffeine} cache with {@code expireAfterAccess}, so idle keys
 * are evicted rather than accumulating forever (an unbounded per-IP map would be a slow memory
 * leak / DoS vector). Eviction is safe: a bucket idle for longer than its refill window has
 * already refilled to full capacity, so dropping it is equivalent to a fresh full bucket — no
 * meaningful rate-limit state is lost. An actively-abusing key keeps hitting the cache and so
 * keeps its (throttled) bucket alive.
 *
...
public class RateLimitService {
    ...
    public ConsumptionProbe tryConsume(Scope scope, String key) {
        ...
        RateLimitProperties.Limit limit = scope == Scope.AUTH ? properties.getAuth() : properties.getSdk();
    ...
    private Bucket newBucket(RateLimitProperties.Limit limit) {
        return Bucket.builder()
                .addLimit(b -> b.capacity(limit.getCapacity())
