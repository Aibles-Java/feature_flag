...
/**
 * Base for the rate-limit filters (issue #26). Subclasses supply the scope and the key a request is
 * bucketed by; this class handles the enabled check, consumption, and emitting a {@code 429 Too
 * Many Requests} with a {@code Retry-After} header.
 *
 * <p>The 429 body is an RFC-7807 {@link ProblemDetail} matching the shape produced by {@code
 * GlobalExceptionHandler} (about:blank type, title, detail, instance). Because filters run before
 * Spring MVC, the response is written here directly rather than via {@code @RestControllerAdvice} —
...
public abstract class AbstractRateLimitFilter extends OncePerRequestFilter {
  ...
  protected AbstractRateLimitFilter(
      RateLimitService rateLimitService, RateLimitService.Scope scope) {
    this.rateLimitService = rateLimitService;
    this.scope = scope;
  }

  /**
   * The bucket key for this request (e.g. client IP or environment id); {@code null} to skip
   * limiting.
   */
