...
public interface FlagEnvironmentStateRepository extends JpaRepository<FlagEnvironmentState, UUID> {
    Optional<FlagEnvironmentState> findByFeatureFlagIdAndEnvironmentId(UUID featureFlagId, UUID environmentId);

    @Query("SELECT s FROM FlagEnvironmentState s JOIN FETCH s.featureFlag f WHERE s.environment.id = :envId AND f.archived = false")
