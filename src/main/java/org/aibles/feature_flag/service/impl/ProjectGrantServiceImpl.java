...
public class ProjectGrantServiceImpl implements ProjectGrantService {
  ...
  public ProjectGrantResponse upsertGrant(UUID projectId, CreateProjectGrantRequest request) {
    ...
    if (builtIn) {
      ...
      customRole =
          customRoleRepository
              .findById(request.getCustomRoleId())
              .orElseThrow(
                  () -> new ResourceNotFoundException("CustomRole", request.getCustomRoleId()));
    ...
    if (!memberRepository.existsByOrganizationIdAndUserId(
        project.getOrganization().getId(), user.getId())) {
  ...
  public void revokeGrant(UUID projectId, UUID userId) {
    ...
    PermissionGrant grant =
        grantRepository
            ...
            .orElseThrow(
                () -> new ResourceNotFoundException("Grant not found for this project and user"));
