 ...
 *
 * <p>Reuse detection revokes the family and then throws {@code InvalidRefreshTokenException}, an
 * unchecked exception that rolls back the ambient transaction by default — silently undoing the
 * revoke. Marking the throwing method {@code noRollbackFor} only fixes the innermost transaction:
 * any caller that is itself {@code @Transactional} (e.g. {@code AuthServiceImpl.refresh}) becomes
 * the outer transaction whose rollback rules win, and the hole reopens.
 *
