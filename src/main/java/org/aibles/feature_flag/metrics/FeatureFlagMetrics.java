 ...
 *
 * <p>The bounded meters ({@link #FLAG_CHANGES}, {@link #AUTH_FAILURES}) are eagerly pre-registered
 * at zero in the constructor; the per-environment evaluation meters are registered lazily on first
...
public class FeatureFlagMetrics {
  ...
  static final String EVALUATION_DURATION =
      "ff.evaluation.duration"; // → ff_evaluation_duration_seconds
  ...
  /**
   * Times an SDK evaluation for {@code environmentId} and counts it. The timer emits {@code
   * ff_evaluation_duration_seconds_{count,sum,max}}; the separate counter satisfies the explicit
   * {@code ff_evaluations_total} metric in the issue's acceptance criteria.
   */
