...
/**
 * JWT signing configuration, bound from {@code app.jwt.*} (env: {@code APP_JWT_SECRET},
 * {@code APP_JWT_EXPIRATION_MS}). Validated at startup so a misconfigured secret aborts
 * boot with a clear binding-failure report instead of running with a weak/known key.
 */
...
public record JwtProperties(
    ...
    /**
     * Substrings of well-known placeholder secrets, including the value this repo used
     * to commit ("change-me-..."). Must not match the intentional local-dev default in
    ...
    /**
     * Minimum distinct characters. A crude entropy proxy that rejects trivially
     * repetitive keys ("xxxx...") while always passing real random keys — hex output
     * of `openssl rand -hex 64` draws from 16 characters, so stay safely below that.
     */
    private static final int MIN_DISTINCT_CHARS = 10;

    @AssertTrue(message = "app.jwt.secret is an unresolved ${...} placeholder — "
    ...
    @AssertTrue(message = "app.jwt.secret must be at least 64 bytes (512 bits) when UTF-8 encoded — "
    ...
    @AssertTrue(message = "app.jwt.secret is a placeholder value — set APP_JWT_SECRET to a real secret")
    ...
    @AssertTrue(message = "app.jwt.secret looks trivially low-entropy (too few distinct characters) — "
